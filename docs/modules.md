...
| Sipil | ✅ | Table, klik baris → buka detail modal; filter status (Active/Building/Finishing/Finished/All) — "Active" adalah frontend alias untuk building + finishing, default view; status diambil langsung dari field `status` PocketBase (building/finishing/finish) |
