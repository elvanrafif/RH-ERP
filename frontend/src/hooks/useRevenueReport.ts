...
import type {
  ReportPeriod,
  ProjectTypeFilter,
} from '@/lib/invoicing/reportCalculations'
...
export function useRevenueReport(
  ...
  const { data: quotations = [], isLoading: loadingQuotations } = useQuery({
    queryKey: ['report-quotations', 'full'],
    queryFn: () =>
      pb
  ...
  const reportData = isLoading
    ? null
    : buildRevenueReportData(invoices, quotations, period, projectType)
