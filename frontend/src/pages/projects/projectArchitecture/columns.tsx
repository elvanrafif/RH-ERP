...
const STATUS_COLORS: Record<string, string> = {
  ...
  denah: 'bg-blue-50 text-blue-700 border-blue-200',
  fasad: 'bg-violet-50 text-violet-700 border-violet-200',
  detail_drawing: 'bg-amber-50 text-amber-700 border-amber-200',
}

export const getArchitectureColumns = (
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col gap-1 min-w-[90px]">
          <Badge variant="outline" className="bg-slate-50 max-w-max font-normal text-slate-600 px-1.5 h-5">
            Land: {lt}m²
          </Badge>
          <Badge variant="outline" className="bg-slate-50 max-w-max font-normal text-slate-600 px-1.5 h-5">
            Building: {lb}m²
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2 min-w-[120px]">
          ...
          <span className="text-sm text-slate-600 truncate max-w-[120px]" title={assignee?.name}>
            {assignee?.name || '—'}
          </span>
        </div>
      )
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col gap-1">
          ...
          >
            {MaskingTextByArchitectureStatus(status)}
          </Badge>
