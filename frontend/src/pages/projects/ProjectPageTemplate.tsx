...
import type { Project, User } from "@/types" // Pastikan import User ada
...
import { Plus, LayoutGrid, Table as TableIcon, Loader2, Banknote, Activity, AlertCircle, Search, X, Filter } from "lucide-react"
...
export default function ProjectPageTemplate({ 
  ...
  const [filterPic, setFilterPic] = useState("all"); // State baru untuk PIC
  ...
  // 2. FETCH USERS (Untuk Dropdown Filter PIC)
  const { data: users } = useQuery({
    queryKey: ['users-list'],
    queryFn: async () => await pb.collection('users').getFullList<User>({ sort: 'name' }),
  });

  // 3. FILTERING LOGIC (Dipindah ke atas Stats agar bisa dipakai Stats)
  const filteredProjects = useMemo(() => {
    ...
    // B. Filter by PIC (Database Assignee)
    if (filterPic !== 'all') {
    ...
    return result;
  }, [projects, searchQuery, filterPic]);

  // 4. STATS LOGIC (Sekarang menghitung dari filteredProjects)
  const stats = useMemo(() => {
    // Default values
    let totalValue = 0, activeCount = 0, urgentCount = 0;
    
    // Gunakan filteredProjects, bukan projects mentah
    // Agar widget ikut berubah sesuai filter PIC
    ...
    sourceData.forEach(p => {
        // Hanya hitung statistik untuk project yang belum selesai
  ...
  }, [filteredProjects]); // Dependency ke filteredProjects
  ...
  const resetFilters = () => { setSearchQuery(""); setFilterPic("all"); }
    ...
    <div className="flex-1 h-full p-4 md:p-8 pt-6 flex flex-col overflow-hidden bg-slate-50/30">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 shrink-0">
            <div>
                ...
                <p className="text-sm text-muted-foreground">Monitoring project tipe {projectType}.</p>
            </div>
            <div className="flex items-center gap-3">
                <div className="flex items-center space-x-2 bg-white p-1.5 rounded-lg border shadow-sm px-3">
                    ...
                    <Label htmlFor="show-done" className="text-xs cursor-pointer">Show History</Label>
                </div>
                <Button onClick={handleCreate} className="bg-primary shadow-sm h-9 text-sm"><Plus className="mr-2 h-4 w-4" /> Baru</Button>
            </div>
        </div>
        ...
        <div className="grid gap-3 grid-cols-1 sm:grid-cols-3 mb-4 shrink-0">
            ...
            <Card className="relative overflow-hidden shadow-sm border-l-4 border-l-emerald-500 p-3 flex items-center gap-4 bg-white">
                ...
                <div className="overflow-hidden">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider truncate">Potensi Omzet</p>
            ...
            <Card className="relative overflow-hidden shadow-sm border-l-4 border-l-blue-500 p-3 flex items-center gap-4 bg-white">
                ...
                <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Project Aktif</p>
                    <div className="text-lg font-bold text-slate-800 flex items-baseline gap-1">
                        {stats.activeCount} <span className="text-xs font-normal text-muted-foreground">Unit</span>
                    </div>
                </div>
            </Card>

            {/* Widget 3: Urgent Deadline (< 7 Hari) */}
            <Card className={`relative overflow-hidden shadow-sm border-l-4 p-3 flex items-center gap-4 transition-colors ${
                stats.urgentCount > 0 
                ? "border-l-red-500 bg-red-50/50" 
                : "border-l-slate-300 bg-white"
            }`}>
                <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${
                    stats.urgentCount > 0 ? "bg-red-100 text-red-600 animate-pulse" : "bg-slate-100 text-slate-500"
                }`}>
                ...
                <div>
                    ...
                    <div className={`text-lg font-bold flex items-baseline gap-1 ${
                        ...
                        {stats.urgentCount} <span className="text-xs font-normal opacity-70">Project</span>
                    </div>
                </div>
            </Card>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3 items-start sm:items-center justify-between shrink-0">
            <div className="flex flex-1 gap-2 w-full sm:w-auto">
                ...
                <div className="relative flex-1 sm:max-w-xs">
                    ...
                    <Input placeholder="Cari client, scope..." className="pl-8 h-9 bg-white" 
                ...
                {/* RESET BUTTON */}
                {hasActiveFilters && (
                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={resetFilters} title="Reset Filter">
        ...
        <div className="flex-1 overflow-hidden relative bg-white/50 rounded-lg border border-slate-200/60 shadow-inner">
             ...
             <Tabs defaultValue={enableKanban ? "kanban" : "table"} className="flex flex-col h-full">
                 ...
                 {isLoading ? <div className="flex h-full items-center justify-center"><Loader2 className="animate-spin text-muted-foreground" /></div> : (
                    <>
                        {enableKanban && (
                        ...
                        <TabsContent value="table" className="flex-1 overflow-auto mt-0 h-full p-0">
                            <ProjectTable projectType={projectType} onView={(project) => setProjectToView(project)} data={filteredProjects} onEdit={handleEdit} onDelete={(p) => setDeleteId(p.id)} />
                        </TabsContent>
                    </>
                )}
            </Tabs>
