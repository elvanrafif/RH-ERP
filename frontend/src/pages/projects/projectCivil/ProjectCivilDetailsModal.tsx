...
interface ProjectCivilDetailsModalProps {
  project: Project | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ProjectCivilDetailsModal({
  project,
  open,
  onOpenChange,
}: ProjectCivilDetailsModalProps) {
  ...
  const fieldPicName = project.expand?.vendor?.name
  const managedByName = project.expand?.assignee?.name
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-[95vw] sm:max-w-[700px] max-h-[90vh] flex flex-col p-0 gap-0">
          ...
          <ProjectModalHero
            project={project}
            projectTypeName="Civil Construction"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border">
              ...
              <div className="px-6 py-5">
                <ProjectPicTimelineCard
                  picData={fieldPicName}
                  isCivil={true}
                  isInterior={false}
                  managedByData={managedByName}
                  project={project}
                />
              </div>
            </div>
