...
import { 
    CalendarClock, User, Ruler, StickyNote, 
    Building2, Phone, Mail, MapPin, Maximize2, Banknote, CalendarRange, ArrowRight
} from "lucide-react"
import { formatDateLong, formatRupiah, getAvatarUrl, getInitials } from "@/lib/helpers"
...
export function ProjectDetailsModal({ project, open, onOpenChange }: ProjectDetailsModalProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            {/* FIX RESPONSIVE:
               1. max-h-[90vh]: Membatasi tinggi modal 90% layar (ada celah atas bawah).
               2. flex flex-col: Agar kita bisa atur header, body, footer.
               3. p-0: Padding kita atur manual per bagian agar scrollbar rapi.
            */}
            <DialogContent className="w-[95vw] sm:max-w-[750px] max-h-[90vh] flex flex-col p-0 bg-white gap-0">
                ...
                <div className="p-6 pb-2 shrink-0">
                    <DialogHeader>
                        <div className="flex items-center gap-2 mb-2">
                            <Badge variant="outline" className="uppercase bg-white border-primary/20 text-primary shadow-sm tracking-wide text-[10px] h-6 px-3">
                                {project.type}
                            </Badge>
                            <Badge variant="secondary" className="uppercase text-[10px] h-6 px-3 bg-slate-200 text-slate-700 hover:bg-slate-300">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-3 flex-1">
                                    <div>
                                        <h3 className="text-lg font-bold text-slate-900 leading-tight">
                                            {client?.company_name || "Tanpa Nama Perusahaan"}
                                        </h3>
                                    </div>
                                    <div className="space-y-2 pt-2 border-t border-slate-100">
                                        <div className="flex items-start text-xs text-slate-600">
                                            ...
                                            <span className="leading-tight">{(client as any)?.address || "Alamat tidak tersedia."}</span>
                                        </div>
                            ...
                            <div className="bg-white p-0 rounded-xl border shadow-sm relative overflow-hidden flex flex-col h-full">
                                ...
                                {/* Bagian Bawah: Timeline (Background Beda) */}
                                <div className="bg-indigo-50/50 p-4 border-t border-indigo-100 flex-1 flex flex-col justify-center">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-xs font-semibold text-indigo-900 flex items-center">
                                            <CalendarClock className="h-3.5 w-3.5 mr-1.5" /> Target Deadline
                                        </span>
                                    ...
                                    {project.type === 'sipil' && (
                                        <div className="text-[10px] text-slate-600 bg-white/60 p-2 rounded border border-indigo-100 mt-1">
                        ...
                        {/* === ROW 2: SPESIFIKASI & NOTES (MERGED CARD) === */}
                        <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                            ...
                            <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                {/* KOLOM KIRI (1/3): DATA TEKNIS */}
                                <div className="md:col-span-1 space-y-3 border-r-0 md:border-r border-slate-100 pr-0 md:pr-4 pb-4 md:pb-0 border-b md:border-b-0">
                                    {project.type != 'interior' && 
                                    <div className="grid grid-cols-2 gap-2">
                                        <div className="bg-slate-50 p-2 rounded text-center border border-slate-100">
                                            <span className="flex items-center justify-center text-[9px] text-muted-foreground uppercase mb-1">
                                                <Maximize2 className="h-3 w-3 mr-1" /> Luas Tanah
                                            </span>
                                            <span className="font-bold text-slate-700 text-sm">{meta.luas_tanah || "-"} <span className="text-[10px] font-normal">m²</span></span>
                                        </div>
                                        <div className="bg-slate-50 p-2 rounded text-center border border-slate-100">
                                            <span className="flex items-center justify-center text-[9px] text-muted-foreground uppercase mb-1">
                                                <Building2 className="h-3 w-3 mr-1" /> Luas Bangunan
                                            </span>
                                            <span className="font-bold text-slate-700 text-sm">{meta.luas_bangunan || "-"} <span className="text-[10px] font-normal">m²</span></span>
                                        </div>
                                    </div>
                                    }
                                    ...
                                    {meta.area_scope && (
                                        <div className="bg-emerald-50/50 p-3 rounded border border-emerald-100">
                                            <span className="block text-[10px] text-emerald-600 font-bold uppercase mb-1">Scope Area</span>
                                            <p className="text-xs font-medium leading-relaxed text-emerald-900">{meta.area_scope}</p>
                                        </div>
                                    )}
                                </div>

                                {/* KOLOM KANAN (2/3): NOTES */}
                                <div className="md:col-span-2 relative">
                                    ...
                                    <div className="mt-5 h-full min-h-[80px] text-xs leading-relaxed text-slate-600 whitespace-pre-line p-1">
                                        {notes || "Tidak ada catatan khusus untuk proyek ini."}
                                    </div>
                                </div>
                ...
                <div className="p-6 pt-2 shrink-0 flex justify-end bg-white/50 backdrop-blur-sm border-t mt-auto">
                    <Button variant="outline" onClick={() => onOpenChange(false)}>Tutup Detail</Button>
                </div>
