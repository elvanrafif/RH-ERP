import { TrendingUp, TrendingDown, Receipt, FileText, DollarSign } from 'lucide-react'
...
export function RevenueStatCards({ data }: RevenueStatCardsProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {cards.map(card => (
        <div key={card.label} className="bg-white rounded-lg border border-border shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className={`p-2 rounded-md ${card.bg}`}>{card.icon}</div>
            <ChangeBadge pct={card.change} />
          </div>
          <p className="text-sm text-muted-foreground">{card.label}</p>
          <p className="text-2xl font-bold text-slate-800 mt-0.5">{formatRupiah(card.value)}</p>
        </div>
      ))}
    </div>
  )
