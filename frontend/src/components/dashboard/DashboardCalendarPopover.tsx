...
const IS_PROJECT_TYPE = new Set(['architecture', 'civil', 'interior'])
    ...
    <div className="w-72 rounded-lg border bg-white shadow-lg overflow-hidden">
      ...
      <div className="divide-y max-h-72 overflow-y-auto">
        {events.length === 0 ? (
          ...
          events.map((event) => {
            const isProject = IS_PROJECT_TYPE.has(event.extendedProps.eventType)
            return (
              <div key={event.id} className="px-4 py-3 space-y-1.5">
                {/* Badge row — bullet and badge aligned */}
