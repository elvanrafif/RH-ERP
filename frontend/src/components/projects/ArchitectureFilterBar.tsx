...
interface ArchitectureFilterBarProps {
  searchQuery: string
...
export function ArchitectureFilterBar({
  searchQuery,
  onSearchChange,
  filterPic,
  onFilterPicChange,
  ...
  users,
  className,
}: ArchitectureFilterBarProps) {
  const picActive = filterPic && filterPic !== 'all'
  const deadlineActive = filterDeadline !== 'all'
  const statusActive = filterStatus !== 'all'
  const archUsers = users.filter(
    (u) => u.division?.toLowerCase() === PROJECT_TYPE_TO_DIVISION['architecture']
  )
