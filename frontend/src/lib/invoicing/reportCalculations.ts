...
export type { Granularity, ProjectTypeFilter, ReportPeriod, BarChartEntry, StatCardData, InvoiceReportRow, QuotationReportRow, RevenueReportData }
...
function parseItems(raw: unknown): TermItem[] {
  ...
  if (typeof raw === 'string') {
    try { return JSON.parse(raw) as TermItem[] } catch { return [] }
  }
  return []
}

function isInPeriod(dateStr: string | undefined, period: ReportPeriod): boolean {
  ...
  if (period.granularity === 'quarterly') {
    return d.getFullYear() === period.year && Math.ceil((d.getMonth() + 1) / 3) === period.quarter
  }
  return d.getFullYear() === period.year
}

function matchesProjectType(docType: string, filter: ProjectTypeFilter): boolean {
  if (filter === 'all') return true
  if (filter === 'architecture') return docType === 'design'
  if (filter === 'civil') return docType === 'sipil'
...
export function getPreviousPeriod(period: ReportPeriod): ReportPeriod {
  if (period.granularity === 'monthly') {
    ...
    return { ...period, month: prevMonth, year: prevYear, quarter: Math.ceil(prevMonth / 3) }
  }
  if (period.granularity === 'quarterly') {
    const prevQ = period.quarter === 1 ? 4 : period.quarter - 1
    const prevYear = period.quarter === 1 ? period.year - 1 : period.year
    return { ...period, quarter: prevQ, year: prevYear }
  }
  return { ...period, year: period.year - 1 }
}

export function getInvoiceRevenue(invoices: any[], period: ReportPeriod, projectType: ProjectTypeFilter): number {
  let total = 0
  for (const inv of invoices) {
    if (!matchesProjectType(inv.type ?? '', projectType)) continue
    for (const item of parseItems(inv.items)) {
      if (item.status === PAYMENT_ITEM_STATUS.SUCCESS && isInPeriod(item.paymentDate, period)) {
...
export function getQuotationRevenue(quotations: any[], period: ReportPeriod): number {
  return quotations
    .filter(q => q.status === QUOTATION_STATUS.PAID && isInPeriod(q.updated, period))
...
export function buildBarChartData(
  ...
  if (period.granularity === 'monthly') {
    return MONTH_LABELS.map((label, idx) => {
      const p: ReportPeriod = { granularity: 'monthly', year: period.year, month: idx + 1, quarter: Math.ceil((idx + 1) / 3) }
      const invoiceValue = getInvoiceRevenue(invoices, p, projectType)
      const quotationValue = getQuotationRevenue(quotations, p)
      return { label, invoiceValue, quotationValue, isCurrentPeriod: period.month === idx + 1, isEmpty: invoiceValue === 0 && quotationValue === 0 }
    })
  }
  if (period.granularity === 'quarterly') {
    return [1, 2, 3, 4].map(q => {
      const p: ReportPeriod = { granularity: 'quarterly', year: period.year, month: (q - 1) * 3 + 1, quarter: q }
      const invoiceValue = getInvoiceRevenue(invoices, p, projectType)
      const quotationValue = getQuotationRevenue(quotations, p)
      return { label: `Q${q}`, invoiceValue, quotationValue, isCurrentPeriod: period.quarter === q, isEmpty: invoiceValue === 0 && quotationValue === 0 }
    })
  }
  // yearly: last 5 years ending at period.year
  return Array.from({ length: 5 }, (_, i) => {
    const y = period.year - 4 + i
    const p: ReportPeriod = { granularity: 'yearly', year: y, month: 1, quarter: 1 }
    const invoiceValue = getInvoiceRevenue(invoices, p, projectType)
    const quotationValue = getQuotationRevenue(quotations, p)
    return { label: String(y), invoiceValue, quotationValue, isCurrentPeriod: period.year === y, isEmpty: invoiceValue === 0 && quotationValue === 0 }
  })
}

export function buildInvoiceRows(invoices: any[], period: ReportPeriod, projectType: ProjectTypeFilter): InvoiceReportRow[] {
  const rows: InvoiceReportRow[] = []
  for (const inv of invoices) {
    if (!matchesProjectType(inv.type ?? '', projectType)) continue
    const terminValue = parseItems(inv.items)
      .filter(item => item.status === PAYMENT_ITEM_STATUS.SUCCESS && isInPeriod(item.paymentDate, period))
...
export function buildQuotationRows(quotations: any[], period: ReportPeriod): QuotationReportRow[] {
  return quotations
    .filter(q => q.status === QUOTATION_STATUS.PAID && isInPeriod(q.updated, period))
    .map(q => {
...
export function buildRevenueReportData(
  ...
  return {
    statCards: {
      ...
      invoiceRevenue,
      quotationRevenue,
      totalChange: calcChange(invoiceRevenue + quotationRevenue, prevInvoice + prevQuotation),
