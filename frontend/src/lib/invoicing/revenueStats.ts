...
export function buildInvoiceRevenueData(
  ...
  const distribution = { architecture: 0, civil: 0, interior: 0, uncategorized: 0 }
  let totalActual = 0
  ...
  const validInvoices = invoices.filter((inv) => {
    ...
    const lastSuccessItem = [...items]
      .reverse()
      .find((item: any) => item.status && item.status.toLowerCase() === 'success')
  ...
  validInvoices.forEach((inv) => {
    ...
    totalActual += amount
    const category = String(inv.type || inv.category || inv.division || '').toLowerCase().trim()
    if (category === 'design') distribution.architecture += amount
    else if (category === 'civil' || category === 'sipil') distribution.civil += amount
  ...
  const topInvoices = [...validInvoices]
    .sort((a, b) => (b.total_amount || b.amount || 0) - (a.total_amount || a.amount || 0))
    .slice(0, 5)
